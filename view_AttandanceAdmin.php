...
<body>
    ...
    <h3 style="text-align: center;">Search Attendance by Student ID</h3>
    ...
    <form method="post" class="frm">
        ...
        <br>
        <?php
    $sql_periods = "SELECT DISTINCT period FROM at WHERE teacher_id = '$teacher_id' AND class_id = '$class_id' AND date(date_time) = '$attendance_date'";
    $result_periods = $conn->query($sql_periods);

    if ($result_periods->num_rows > 0) {
        while($period_row = $result_periods->fetch_assoc()) {
            $period = $period_row['period'];
            echo "<h3>Period $period</h3>";
            $sql_attendance = "SELECT * FROM at WHERE teacher_id = '$teacher_id' AND class_id = '$class_id' AND date(date_time) = '$attendance_date' AND period = '$period'";
            ...
            if ($result_attendance->num_rows > 0) {
                ...
                echo "No attendance records found for Period $period.";
            }
        }
    } else {
        echo "No attendance records found for the selected date and class ID.";
    }
    ?>
    </form>
